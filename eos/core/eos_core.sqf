...
if (!_cache) then {
	if ismultiplayer then {
			...
			{_actCond="{vehicle _x in thisList && isplayer _x && ((getPosATL _x) select 2) < 5 } count playableunits > 0 or {getConnectedUav _x in thisList} count allplayers > 0 ";
							}else 
							{_actCond="{vehicle _x in thisList && isplayer _x } count playableunits > 0 or {getConnectedUav _x in thisList} count allplayers > 0";
		};}else{
			if (_heightLimit) then 
						{_actCond="{vehicle _x in thisList && isplayer _x && ((getPosATL _x) select 2) < 5 } count allUnits > 0 or {getConnectedUav _x in thisList} count allplayers > 0";
								}else
									{_actCond="{vehicle _x in thisList && isplayer _x } count allUnits > 0 or {getConnectedUav _x in thisList} count allplayers > 0";};};
...
//SPAWN ALT TRIGGERS	
			...
			_clear setTriggerStatements ["this","",""]; 
				...
				_taken setTriggerStatements ["{vehicle _x in thisList && isplayer _x && ((getPosATL _x) select 2) < 5 ","",""]; 
_eosAct=true;	
while {_eosAct} do
	{
